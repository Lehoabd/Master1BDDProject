-- Désactivation des contraintes pour suppression
...
CREATE TABLE Club (
    ...
    region VARCHAR(100),
    numDirecteur INT UNIQUE,
    FOREIGN KEY (numDirecteur) REFERENCES Utilisateur(numUtilisateur) ON DELETE SET NULL
);

-- Table Utilisateur (base pour les autres entités)
...
-- Table Directeur (hérite de Utilisateur)
...
-- Table President (hérite de Utilisateur)
...
-- Table Competiteur (hérite de Utilisateur)
...
-- Table Evaluateur (hérite de Utilisateur)
...
CREATE TABLE Concours (
    ...
    FOREIGN KEY (numPresident) REFERENCES President(numPresident) ON DELETE SET NULL
);
...
-- Table Participation des Compétiteurs
